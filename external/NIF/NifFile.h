...
public:
    ...
    NifFile(const std::string &fileName, const NifLoadOptions &options = NifLoadOptions()) { Load(fileName, options); }

    NifFile(std::fstream &file, const NifLoadOptions &options = NifLoadOptions()) { Load(file, options); }

    NifFile(const NifFile &other) { CopyFrom(other); }

    NifFile &operator=(const NifFile &other) { CopyFrom(other); }
    ...
    int Load(Stream &file, const NifLoadOptions &options = NifLoadOptions());
    int Save(const std::string &fileName, const NifSaveOptions &options = NifSaveOptions());
    template<class Stream>
    int Save(Stream &file, const NifSaveOptions &options = NifSaveOptions());
