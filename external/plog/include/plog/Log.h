...
#if defined(PLOG_ENABLE_GET_THIS) && defined(_MSC_VER) && _MSC_VER >= 1600 && !defined(__INTELLISENSE__) \
    && !defined(__INTEL_COMPILER) && !defined(__llvm__)                                                  \
    && !defined(                                                                                         \
        __RESHARPER__) // >= Visual Studio 2010, skip IntelliSense, Intel Compiler, Clang Code Model and ReSharper
...
#define PLOG_(instanceId, severity)                                                                        \
    ...
    (*plog::get<instanceId>())                                                                             \
        += plog::Record(severity, PLOG_GET_FUNC(), __LINE__, PLOG_GET_FILE(), PLOG_GET_THIS(), instanceId) \
               .ref()
