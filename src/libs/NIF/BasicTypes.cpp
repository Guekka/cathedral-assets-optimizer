...
constexpr auto NIF_GAMEBRYO   = "Gamebryo File Format";
constexpr auto NIF_NETIMMERSE = "NetImmerse File Format";
constexpr auto NIF_NDS        = "NDSNIF....@....@....";
constexpr auto NIF_VERSTRING  = ", Version ";
...
void NiHeader::DeleteBlock(int blockId)
{
    ...
    for (unsigned short blockTypeIndice : blockTypeIndices)
...
void NiHeader::SetBlockOrder(std::vector<std::pair<int, int>> &newIndices)
{
    ...
    newIndices.erase(std::remove_if(newIndices.begin(),
                                    newIndices.end(),
                                    [](auto &p) { return p.first == 0xFFFFFFFF; }),
                     newIndices.end());
...
{
    NiString niStr;
    auto typeId = static_cast<ushort>(blockTypes.size());
...
void NiHeader::Get(NiStream &stream)
{
    ...
    bool isNetImmerse = std::strstr(ver, NIF_NETIMMERSE) != nullptr;
    bool isGamebryo   = std::strstr(ver, NIF_GAMEBRYO) != nullptr;
    bool isNDS        = std::strstr(ver, NIF_NDS) != nullptr;
    ...
    auto verStrPtr = std::strstr(ver, NIF_VERSTRING);
    if (verStrPtr)
    {
