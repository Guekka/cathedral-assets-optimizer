...
void NiGeometryData::Create(const std::vector<Vector3> *verts,
...
int NiShape::GetSkinInstanceRef() const
{
    return 0xFFFFFFFF;
}
...
void BSTriShape::Get(NiStream &stream)
{
    ...
    if (dataSize > 0)
    {
        ...
        for (int i = 0; i < numVertices; i++)
        {
            ...
            if (HasNormals())
            {
                for (unsigned char &j : vertex.normal)
                    stream >> j;
                ...
                if (HasTangents())
                {
                    for (unsigned char &j : vertex.tangent)
                        stream >> j;
            ...
            if (HasVertexColors())
                for (unsigned char &j : vertex.colorData)
                    stream >> j;
            ...
            if (IsSkinned())
            {
                ...
                for (unsigned char &weightBone : vertex.weightBones)
                    stream >> weightBone;
            }
...
void BSTriShape::RecalcNormals(const bool smooth, const float smoothThresh)
{
    ...
    for (int i = 0; i < numVertices; i++)
    {
        ...
        vertData[i].normal[0] = static_cast<unsigned char>(
            round((((rawNormals[i].x + 1.0F) / 2.0F) * 255.0F)));
        vertData[i].normal[1] = static_cast<unsigned char>(
            round((((rawNormals[i].y + 1.0F) / 2.0F) * 255.0F)));
        vertData[i].normal[2] = static_cast<unsigned char>(
            round((((rawNormals[i].z + 1.0F) / 2.0F) * 255.0F)));
    }
}
...
void BSTriShape::CalcTangentSpace()
{
    ...
    for (int i = 0; i < numVertices; i++)
    {
        ...
        vertData[i].tangent[0] = static_cast<unsigned char>(
            round((((rawTangents[i].x + 1.0F) / 2.0F) * 255.0F)));
        vertData[i].tangent[1] = static_cast<unsigned char>(
            round((((rawTangents[i].y + 1.0F) / 2.0F) * 255.0F)));
        vertData[i].tangent[2] = static_cast<unsigned char>(
            round((((rawTangents[i].z + 1.0F) / 2.0F) * 255.0F)));

        vertData[i].bitangentX = rawBitangents[i].x;
        vertData[i].bitangentY = static_cast<unsigned char>(
            round((((rawBitangents[i].y + 1.0F) / 2.0F) * 255.0F)));
        vertData[i].bitangentZ = static_cast<unsigned char>(
            round((((rawBitangents[i].z + 1.0F) / 2.0F) * 255.0F)));
    }
}
...
void BSTriShape::Create(const std::vector<Vector3> *verts,
                        const std::vector<Triangle> *tris,
...
void BSSubIndexTriShape::Create(const std::vector<Vector3> *verts,
                                const std::vector<Triangle> *tris,
                                const std::vector<Vector2> *uvs,
                                const std::vector<Vector3> *normals)
{
...
void BSDynamicTriShape::Create(const std::vector<Vector3> *verts,
                               const std::vector<Triangle> *tris,
                               const std::vector<Vector2> *uvs,
                               const std::vector<Vector3> *normals)
{
...
void NiTriBasedGeomData::Create(const std::vector<Vector3> *verts,
...
void NiTriShapeData::Create(const std::vector<Vector3> *verts,
                            const std::vector<Triangle> *inTris,
                            const std::vector<Vector2> *texcoords,
                            const std::vector<Vector3> *norms)
{
