...
[[nodiscard]] auto find_matching_paths_icase(
    const btu::Path &directory, std::span<const btu::Path> paths) noexcept -> std::vector<btu::Path>
{
#ifdef _WIN32
    ...
    const auto files_in_directory = flux::from_range(btu::fs::recursive_directory_iterator(directory))
                                        .map([&directory](const btu::fs::directory_entry &entry) {
                                            const auto &path         = entry.path();
                                            const auto relative_path = btu::fs::relative(path, directory);
                                            return std::pair{btu::common::to_lower(relative_path.u8string()),
                                                             path};
                                        })
 ...
 */
auto remove_files_from_directory(const btu::Path &directory,
                          ...
                          const bool remove_files)
{
    ...
    if (remove_files)
    {
        ...
        PLOGI << fmt::format("Attempted to remove {} files, {} were removed", relative_paths.size(), count);
    }
}
